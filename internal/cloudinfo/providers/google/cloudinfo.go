...
var regionNames = map[string]string{
	...
	"us-west2":                "Americas (LosAngeles)",
	"us-west3":                "Americas (SaltLakeCity)",
	"us-west4":                "Americas (LasVegas)",
	"us-central1":             "Americas (Iowa)",
	"us-east1":                "Americas (SouthCarolina)",
	"us-east4":                "Americas (N.Virginia)",
	...
	"asia-east2":              "Asia Pacific (HongKong)",
...
func (g *GceInfoer) getPrice() (map[string]map[string]map[string]float64, error) {
	...
	nextPageToken := ""
	for {
		svcList, err := g.cbSvc.Services.List().PageToken(nextPageToken).Fields("services/displayName", "services/name", "nextPageToken").Do()
		if err != nil {
			return nil, err
		}
